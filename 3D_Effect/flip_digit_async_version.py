...
class FlipDigit(Widget):
    ...
    def top_flip_animate(self, value, blocking=True):
        ...
        if blocking:
            ...
            self.animating_state = FlipDigit.ANIMATING_TOP
            self.animating = True
            self.top_anim_tilegrid[0] = self.current_animation_frame + (value * 10)
            self.last_animation_time = time.monotonic()
            self.current_animation_frame += 1
            self.top_animating_value = value


    def bottom_flip_animate(self, value, blocking=True):
        ...
        if blocking:
            ...
            self.animating_state = FlipDigit.ANIMATING_BOTTOM
            self.animating = True
            self.bottom_anim_tilegrid[0] = self.current_animation_frame + (value * 10)
            self.last_animation_time = time.monotonic()
            self.current_animation_frame += 1
            self.bottom_animating_value = value


    def animate(self):
        now = time.monotonic()
        if now > self.last_animation_time + self.anim_delay:
            if self.animating_state == FlipDigit.ANIMATING_TOP:
                if self.current_animation_frame < self.anim_frame_count:
                    self.top_anim_tilegrid[0] = self.current_animation_frame + (self.top_animating_value * 10)
                    self.last_animation_time = time.monotonic()
                    self.current_animation_frame += 1
                else:
                    # last frame is done
            ...
            if self.animating_state == FlipDigit.ANIMATING_BOTTOM:
                if self.current_animation_frame < self.anim_frame_count:
                    self.bottom_anim_tilegrid[0] = self.current_animation_frame + (self.bottom_animating_value * 10)
                    self.last_animation_time = time.monotonic()
                    self.current_animation_frame += 1
                else:
                    # last frame is done
